...
class GroqClient:
    ...
    def resume_cv(self, cv):
        prompt = f'''
            **Solicitação de Resumo de Currículo em Markdown:**
            ...
            Por favor, gere um resumo do currículo fornecido, 
            formatado em Markdown, seguindo rigorosamente o modelo abaixo. 
            **Não adicione seções extras, tabelas ou qualquer outro tipo de 
            formatação diferente da especificada.** 
    ...
    def generate_score(self, cv, job, max_attempts=10):
        prompt =  f'''
            ...
            **Instruções:**
            ...
            1. **Experiência (Peso: 50%)**: Avalie a relevância da experiência em relação à vaga.
            2. **Habilidades Técnicas (Peso: 20%)**: Verifique o alinhamento das habilidades técnicas com os requisitos da vaga.
            3. **Educação (Peso: 20%)**: Avalie a relevância da formação acadêmica para a vaga.
            4. **Idiomas (Peso: 5%)**: Avalie os idiomas e sua proficiência em relação à vaga.
            5. **Pontos Fortes (Peso: 5%)**: Avalie a relevância dos pontos fortes para a vaga.
    ...
    def generate_opinion(self, cv, job):
        prompt = f'''
            Por favor, analise o currículo fornecido em relação à descrição da vaga aplicada 
            e crie uma opinião ultra crítica e detalhada. 
            A sua análise deve incluir os seguintes pontos:
            Você deve pensar como o recrutador chefe que está analisando e 
            gerando uma opnião descritiva sobre o curriculo do canditato 
            que se candidatou para a vaga
